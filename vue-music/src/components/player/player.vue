  ...
  export default {
    ...
    methods: {
      ...
      next() {
        ...
        if (this.playlist.length === 1) { // 处理歌单只有一首歌的时候的边界情况
          this.loop()
          return
        } else {
      ...
      error() { // 防止歌曲加载失败的时候，点击和上下首失败的回调函数
        this.songReady = true
      },
      updateTime(e) {
      ...
      format(interval) {
        interval = interval | 0 // 或0这个操作符就代表向下取整
      ...
      handleLyric({lineNum, txt}) {
        ...
        if (lineNum > 5) {
          let lineEl = this.$refs.lyricLine[lineNum - 5] // 定义当前滚动元素 - 5 的高度
